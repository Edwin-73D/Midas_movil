...
export default function MetaItem({
  ...
  const progressColor = getProgressColor(meta.porcentajeActual);
  const isComplete = meta.porcentajeActual >= 100;

  const handleDelete = () => {
    Alert.alert(
      'Eliminar meta',
      `¿Seguro que quieres eliminar "${meta.nombre}"?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        { text: 'Eliminar', style: 'destructive', onPress: () => meta.id && onDelete(meta.id) },
      ]
    );
  };
